  ...
  for (voronoi_diagram<double>::const_edge_iterator it = vd.edges().begin();
    ...
    if (it->is_primary()){
        if (it->vertex0()!=nullptr && it->vertex1()!=nullptr){
            ...
            //std::cout<<"_view.drawLine("<<start_x<<","<<start_y<<",";
...
int iterate_primary_edges2(const voronoi_diagram<double> &vd) {
  int result = 0;
  int boundaries_segment_counter = 0;
      ...
      if (edge->is_primary()){
        if (edge->vertex0()!=nullptr && edge->vertex1()!=nullptr){
           ...
           // std::cout<<end_x<<","<<end_y<<");"<<std::endl;
            /*std::cout<<"Edge start point: "<<start_x<<","<<start_y<<std::endl;
            std::cout<<"Edge end point: "<<end_x<<","<<end_y<<std::endl;*/
             //for eliminating the inner boundaries
           // std::cout<<"Boundary_Segment"<<boundaries_segment_counter<<": "<<start_x<<" "<<start_y<<
              //  " "<<end_x<<" "<<end_y<<std::endl;

        }
        ++result;
        boundaries_segment_counter++;
      }
      edge = edge->next();
    ...
    do {
      if (edge->is_primary()){
        if (edge->vertex0()!=nullptr && edge->vertex1()!=nullptr){
            ...
            std::cout<<end_x<<","<<end_y<<");"<<std::endl;*/
            //for eliminating the inner boundaries
            std::cout<<"Boundary_Segment"<<boundaries_segment_counter<<": "<<start_x<<" "<<start_y<<" "<<end_x<<" "<<end_y<<std::endl;
        }
          ++result;
          boundaries_segment_counter++;
      }
...
//16 nets_one_shorter_sides
/*
segments.push_back(Segment(-35811400,50000000, -35811400,65811400));
segments.push_back(Segment(-35811400,65811400, -60000000,90000000));
...
segments.push_back(Segment(43651600,50000000, 83651600,90000000));
segments.push_back(Segment(83651600,90000000, 90000000,90000000));
...
segments.push_back(Segment(-10853000,-50000000, -10853000,-60853000));
segments.push_back(Segment(-10853000,-60853000, -40000000,-90000000));
segments.push_back(Segment(-32073500,-50000000, -32073500,-51046200));
segments.push_back(Segment(-32073500,-51046200, -71027300,-90000000));
segments.push_back(Segment(-71027300,-90000000, -80000000,-90000000));
...
//output segment to _view.drawLine(_sx,_sy,_ex,_ey);
    ...
    /*for(auto segment:segments){
        std::cout<<"_view.drawLine("<<segment.p0.a<<","<<segment.p0.b<<","
            <<segment.p1.a<<","<<segment.p1.b<<");"<<std::endl;
    }*/

    //for eliminating the inner boundaries
    int seg_counter = 0;
    for(auto segment:segments){
        std::cout<<"Segment"<<seg_counter<<": "<<segment.p0.a<<" "<<segment.p0.b<<" "
            <<segment.p1.a<<" "<<segment.p1.b<<" "<<std::endl;
        seg_counter++;
    }
    std::cout<<"all_net_coord_info_end:\n"<<std::endl;
